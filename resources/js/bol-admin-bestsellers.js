...
var BolBestsellersDialog = {
    ...
    init : function() {
        ...
        jqBol('.triggerPreview').change(BolBestsellersDialog.getProductPreview);
    },
    ...
    {
        var val = jqBol('#'+main+' option:selected').val();

        jqBol("#ddl"+sub).next('span.loader').removeClass('hideElement');
        ...
        jqBol.ajax({
            ...
            data: {},
            success: function(response) {
                jqBol("#ddl" + sub).removeAttr('disabled');

                // Check the result of the response
                if (response.indexOf('option') != -1 || response == '') {
                    if (sub.search('SubCategory') >= 0) {
                        jqBol("#ddl"+sub).html("<option value='0'>- " + i10n.selectcategory + " -</option>"+response);
                    } else {
                        jqBol("#ddl"+sub).html("<option value='0'>- " + i10n.selectsubcategory + " -</option>"+response);
                    }
                } else {
                    alert(response);
                }

                jqBol("#ddl"+sub).next('span.loader').addClass('hideElement');
            }
        });
    },
    ...
    _getSelectedCategory : function()
    {
        ...
        var priceRange = jqBol('#priceRangeList').val();
...
if (typeof(tinyMCEPopup) !== 'undefined') {
    ...
    jqBol(document).ready(BolBestsellersDialog.init);
}
